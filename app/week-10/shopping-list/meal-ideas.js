...
//                   (hoveredMeal === meal.idMeal || selectedMeal?.idMeal === meal.idMeal) && 'bg-orange-900'
//                 }`}
//                 onClick={() => loadMealDetails(meal.idMeal)}
//                 onMouseEnter={() => handleMealHover(meal.idMeal)}
...
//       {selectedMeal && (
//         <div>
//           <h3 className="text-xl font-bold mt-8">Ingredients for {selectedMeal.strMeal}</h3>
//           <ul className="list-disc ml-8 mt-4">
//             {Array.from({ length: 20 }, (_, i) => i + 1).map((index) => {
//               const ingredientKey = `strIngredient${index}`;
//               const measureKey = `strMeasure${index}`;
